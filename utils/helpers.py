...
utils/helpers.py
----------------
General-purpose utility functions for file naming, hashing, caching, file filtering, diff display, parallel LLM analysis, git diff, CLI spinners, and .env file management.
...
def safe_name(name: str) -> str:
    """
    Convert a repository or branch name into a filesystem-safe string.
    Replaces slashes and spaces with underscores.
    Args:
        name (str): The original name (e.g., repo or branch).
    Returns:
        str: Filesystem-safe version of the name.
    """
    return name.replace('/', '_').replace(' ', '_')

def get_report_filename(repository: str, branch: str, pull_request_number: int = None) -> str:
    """
    ...
    filename = f"github_audit_{safe_repo}_{safe_branch}"
    if pull_request_number:
        filename += f"_pr{pull_request_number}"
    return filename + ".md"
...
def get_cache_path(repository: str, branch: str, pull_request_number: int, filename_hash: str) -> str:
    """
...
def analyze_files_parallel(files, comments, commits, readme, llm_client, repo_name, branch_name, pr_number, max_workers=8):
    """
    ...
    if uncached:
        with Progress() as progress:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                futures = {
                    executor.submit(analyze_code_files, [f], comments, commits, readme, llm_client): f for f in uncached
                }
